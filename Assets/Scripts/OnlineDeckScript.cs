...
public class OnlineDeckScript : MonoBehaviour{
    ...
    public IEnumerator clearHand(int winner) {
        ...
        for (int j = 0; j < 4; j++)
        {
            if (pHand.cards.Count > 0)
            {
                ...
                {
                    if (pHand.cards[k].GetComponent<CardScript>().rank < onBoardCards[j].rank && pHand.cards[k].GetComponent<CardScript>().kind == onBoardCards[j].kind)
                    {
        ...
        if(winner%2 == 0)
        {
            cardPos = new Vector2(0.4f * friendsHandWon, 3.4f + ((friendsHandWon % 2) * 0.4f));
        }
        else
        {
            cardPos = new Vector2(3f - ((oppsHandsWon % 2) * 0.4f), -3.2f + (oppsHandsWon * 0.4f));
        }
        int b = 0;
        clipCard.Play();
        while (b<10)
